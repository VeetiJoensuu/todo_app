...
const postLogin = async(req, res, next) => {
    const invalid_creditentials_message = 'Invalid creditentials.'
    try {
        const userFromDb = await selectUserByEmail(req.body.email)
        if (userFromDb.rowCount ===0) return next(new ApiError(invalid_creditentials_message))
        
        const user = userFromDb.rows[0]
        if (!await compare(req.body.password, user.password)) return next(new ApiError(invalid_creditentials_message, 401))
